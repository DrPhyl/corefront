...
const pricing = [
  ...
  },
  {
    ...
    features: ["59 generations/month", "All frameworks", "Priority support", "Private projects", "API access"],
  ...
  },
  {
    ...
    price: "$59",
    desc: "For teams",
    features: ["250 generations/month", "Everything in Pro", "Team collaboration", "SSO & SAML", "Dedicated support"],
    cta: "Contact sales",
    featured: false,
  },
];
    ...
    <div className="min-h-screen bg-[#070b14]">
      ...
      <nav
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-200 ${
          scrolled ? "bg-[#070b14]/80 backdrop-blur-xl border-b border-[#1e2d4a]" : ""
        }`}
      >
      ...
      <section className="relative min-h-screen flex flex-col items-center justify-center px-6 pt-16">
          ...
          <div className="max-w-[680px] mx-auto mb-6">
            <div className="relative group">
              ...
              <input
                ...
                className="w-full pl-12 pr-32 py-5 bg-[#0d1427] border border-[#1e2d4a] rounded-full text-[#f0f4ff] placeholder-[#3a4a6b] text-base focus:outline-none focus:border-[#2563eb] focus:shadow-[0_0_30px_rgba(37,99,235,0.15)] transition-all"
              />
              <Link
        ...
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2">
          <div className="w-6 h-10 rounded-full border border-[#1e2d4a] flex items-start justify-center p-2">
            <div className="w-1 h-2 bg-[#3a4a6b] rounded-full animate-bounce" />
          </div>
        </div>
      </section>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div className="card p-6">
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-[#3a4a6b] text-xs ml-2 font-mono">chat</span>
              </div>
              <div className="space-y-4 font-mono text-sm">
                  ...
                  <div key={i} className={`flex items-start gap-3 ${i === typingIndex ? "animate-fade-in" : ""}`}>
                    {line.type === "input" && (
                      <>
                        ...
                        <span className="text-[#f0f4ff]">{line.text}</span>
                      </>
                    )}
                    {line.type === "output" && (
                      <>
                        <span className="text-[#3a4a6b]">│</span>
                        <span className="text-[#7c8db5]">{line.text}</span>
                      </>
                    )}
                    {line.type === "success" && (
                      <>
                ...
                {typingIndex < terminalLines.length && (
                  <span className="inline-block w-2 h-4 bg-[#2563eb] animate-blink" />
                )}
              </div>
            </div>

            <div className="card p-6 bg-[#070b14] border-[#1e2d4a]">
              <div className="flex items-center gap-2 mb-4">
                <span className="w-3 h-3 rounded-full bg-[#ef4444]" />
                <span className="w-3 h-3 rounded-full bg-[#f59e0b]" />
                <span className="w-3 h-3 rounded-full bg-[#10b981]" />
                <span className="text-[#3a4a6b] text-xs ml-2 font-mono">TaskList.tsx</span>
              </div>
              <pre className="font-mono text-sm overflow-x-auto">
                <code>
                  <span className="text-[#3a4a6b]">1  </span><span className="syntax-keyword">export function</span> <span className="syntax-function">TaskList</span>() {"{\n"}
                  <span className="text-[#3a4a6b]">2  </span>  <span className="syntax-keyword">const</span> [tasks, setTasks] = <span className="syntax-function">useState</span>([]){";\n"}
                  <span className="text-[#3a4a6b]">3  </span>{"  \n"}
                  <span className="text-[#3a4a6b]">4  </span>  <span className="syntax-keyword">return</span> ({"{\n"}
                  <span className="text-[#3a4a6b]">5  </span>    {"<"}<span className="syntax-keyword">div</span> className=<span className="syntax-string">"task-list"</span>{">"}{"{\n"}
                  <span className="text-[#3a4a6b]">6  </span>      {"{"}tasks.<span className="syntax-function">map</span>((task) ={">"} ({"{\n"}
                  <span className="text-[#3a4a6b]">7  </span>        {"<"}<span className="syntax-function">TaskItem</span> key={"{"}task.id{"}"} {"/>"}{"\n"}
                  <span className="text-[#3a4a6b]">8  </span>      )){"}\n"}
                  <span className="text-[#3a4a6b]">9  </span>    {"</"}<span className="syntax-keyword">div</span>{">"}{")\n"}
                  <span className="text-[#3a4a6b]">10 </span>  {"}"}{")\n"}
                  <span className="text-[#3a4a6b]">11 </span>{"}"}
                </code>
              </pre>
            </div>
          </div>
      ...
      <section className="py-24 px-6 bg-[#070b14]">
        <div className="max-w-5xl mx-auto">
          <h2 className="font-display text-3xl md:text-4xl text-[#f0f4ff] text-center mb-16">
            How it works
          </h2>
          <div className="relative">
            <div className="hidden md:block absolute top-1/2 left-0 right-0 h-px bg-[#1e2d4a]" />
      ...
      {/* FRAMEWORKS */}
      <section className="py-24 px-6 border-y border-[#1e2d4a] bg-[#0a0f1e]">
        <div className="max-w-4xl mx-auto text-center">
          <h2 className="text-[#3a4a6b] text-sm uppercase tracking-wider mb-8">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            {frameworks.map((fw) => (
              <div
                key={fw.name}
                className="px-5 py-2.5 bg-[#0d1427] border border-[#1e2d4a] rounded-lg hover:border-[#2a4070] transition-colors cursor-default"
              >
      ...
      {/* PRICING */}
      <section id="pricing" className="py-24 px-6 bg-[#070b14]">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {pricing.map((plan) => (
              ...
              >
                <h3 className="text-[#f0f4ff] font-semibold mb-1">{plan.name}</h3>
                <p className="text-[#3a4a6b] text-sm mb-4">{plan.desc}</p>
                <div className="flex items-baseline gap-1 mb-6">
                  <span className="text-[#f0f4ff] text-4xl font-semibold">{plan.price}</span>
                  <span className="text-[#3a4a6b] text-sm">/month</span>
                </div>
                ...
                <Link
                  href="/register"
                  className={`btn w-full ${plan.featured ? "btn-primary" : "btn-outline-blue"}`}
                >
                  {plan.cta}
                </Link>
              </div>
